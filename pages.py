...
def transaksi_show_screen(id_transaksi=None, id_barang=None):
    ...
    if len(model.transactions) == 0:
        ...
        sentences_list.append("Total transaksi setelah diskon: "+locale.currency(
            your_function.calculateTotalPriceAfterDiscount(model.transactions[id_transaksi]["total_harga"]), grouping=True))
        sentences_list.append("Diskon: "+locale.currency(
            total - your_function.calculateTotalPriceAfterDiscount(model.transactions[id_transaksi]["total_harga"]), grouping=True))
        sentences_list.append("")
